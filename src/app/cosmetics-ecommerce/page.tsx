...
import cosmeticsData from "@/data/cosmetics-ecommerce.json";

export default function CosmeticsEcommerceCaseStudy() {
  const sliderImages = Array.from({ length: 6 }, (_, i) => `/case-studies/cosmetics/sl-${i + 1}.png`);
  ...
  const customPreviews = (
    <div id="previews" className="w-full bg-[#f0f4f8] pt-16 pb-16 border-t border-zinc-200 mt-8 shadow-inner overflow-hidden">
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-20 mb-8 text-center">
        ...
        <p className="text-gray-500 max-w-2xl mx-auto">Explore the custom interfaces designed specifically for premium cosmetics shopping.</p>
      </div>

      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-20 pb-10">
        <div className="relative w-full overflow-hidden border border-gray-200/50 rounded-2xl bg-zinc-50/50 py-12 px-4 shadow-inner flex flex-col items-center">

          <div className="relative w-full h-[450px] md:h-[650px] flex justify-center items-center perspective-[1000px]">
            {sliderImages.map((src, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    transform: `translateX(${offset * 60}%) scale(${1 - Math.abs(offset) * 0.15})`,
                ...
                >
                  {/* Image Container matching tall aspect ratio */}
                  <div className="w-[240px] md:w-[320px] h-[400px] md:h-[550px] relative drop-shadow-2xl transition-transform duration-500 hover:scale-105">
                    <Image
                      src={src}
                      alt={`Luxe Beauty Platform Preview ${idx + 1}`}
                      fill
          ...
          {/* Indicators */}
          <div className="mt-14 flex justify-center gap-2 z-10 flex-wrap px-4">
  ...
  return <CaseStudyTemplate data={cosmeticsData as CaseStudyData} customPreviews={customPreviews} />;
}
