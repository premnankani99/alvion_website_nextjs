...
import insurxData from "@/data/insurx.json";

export default function InsurxCaseStudy() {
  const sliderImages = Array.from({ length: 8 }, (_, i) => `/case-studies/insurx/in${i + 1}.png`);
  ...
  const customPreviews = (
    <div id="previews" className="w-full bg-[#f0f4f8] pt-16 pb-16 border-t border-zinc-200 mt-8 shadow-inner overflow-hidden">
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-20 mb-8 text-center">
        ...
        <p className="text-gray-500 max-w-2xl mx-auto">Browse the user-friendly interfaces developed for efficient insurance management and policy processing.</p>
      </div>

      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-20 pb-10">
        <div className="relative w-full overflow-hidden border border-gray-200/50 rounded-2xl bg-zinc-50/50 py-10 px-4 shadow-inner flex flex-col items-center">

          <div className="relative w-full h-[400px] md:h-[550px] flex justify-center items-center perspective-[1000px]">
            {sliderImages.map((src, idx) => {
              ...
              return (
                ...
                >
                  {/* Image Container matching wide landscape aspect ratio */}
                  <div className="w-[320px] md:w-[950px] h-[220px] md:h-[550px] relative drop-shadow-2xl transition-transform duration-500 hover:scale-105">
                    <Image
                      src={src}
                      alt={`InsurX Platform Preview ${idx + 1}`}
                      fill
  ...
  return <CaseStudyTemplate data={insurxData as CaseStudyData} customPreviews={customPreviews} />;
}
